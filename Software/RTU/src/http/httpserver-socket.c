...
/**
  ...
  * @retval None
  */
void http_server_serve(int conn,char *ptr)
{
	struct fs_file * file;
  ...
  else if((strncmp((char *)recv_buffer, "GET /STM32F4x7.html", 19) == 0)||(strncmp((char *)recv_buffer, "GET / ", 6) == 0))
  {
    ...
    {
      /* Load STM32F4x7 page */
      file = fs_open("/upload.html");
      write(conn, (const unsigned char*)(file->data), (size_t)file->len);
      if(file) fs_close(file);
    }
  else if(strncmp((char *)recv_buffer, "GET /uploaddone.html", 20) == 0)
      {
        /* Load STM32F4x7 page */
        file = fs_open("/uploaddone.html");
        write(conn, (const unsigned char*)(file->data), (size_t)file->len);
        if(file) fs_close(file);
      }
  else
...
static void http_server_socket_thread(void *arg)
{
 ...
 /* create a TCP socket */
  ...
  while (1) 
  {
    ...
    http_server_serve(newconn,NULL);
  }
}
