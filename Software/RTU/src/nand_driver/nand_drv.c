...
uint16_t NAND_PostWriteECC(void)
{
  ...
  if (InitialPage != 0)
  {
   ...
   if(COPY_STATE == COPY_BACK_SUPPORTED)
    NAND_CopyBack(WAddress, FAddress, InitialPage);
  else
    NAND_Copy(WAddress, FAddress, InitialPage);
...
*/
                               ...
                               uint16_t PageToCopy)
 {
   ...
   uint16_t count;
   uint16_t loop;
   uint8_t *Copy_Buff=malloc(2048),flag=0;
   switch(NAND_TYPE)
   {
     case (SBLK_NAND):
     ...
     case(LBLK_NAND):
		 endPartialPage  = (Address_Src.Page + PageToCopy)%Multiplier;
		        ...
		        if(startPartialPage >= PageToCopy)
		        {
		         ...
		         fullPage = (PageToCopy - startPartialPage - endPartialPage)/Multiplier;
		       ...
		       ***It will range from 0 to (ACTUAL PAGE SIZE)/512B*/
		       for(count = 0;count<startPartialPage;count++)
		       {
		         ...
		         /*****Copy Back Full pages(2048B)******/
		                ...
		                {
		                	if(!(Address_Src.Block%2==0)^(Address_Dest.Block%2==0))
		                	{
		                	...
		                	{
		                		FSMC_NAND_ReadSmallPage  (Copy_Buff, Address_Src , 4);
		                	    FSMC_NAND_WriteSmallPage (Copy_Buff, Address_Dest, 4);
		                	    flag=1;
...
uint16_t SBLK_NAND_BuildLUT (uint8_t ZoneNumber)
{
  ...
  for(uint16_t count = 0;count<MaxPhyBlockPerZone;count++)
  {
