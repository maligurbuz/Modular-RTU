...
CDC_Xfer_TypeDef                    CDC_GnssRxParam;
...
__ALIGN_BEGIN uint8_t               GnssRxBuf [CDC_BUFFER_SIZE] __ALIGN_END ;
...
static void CDC_ReceiveData(uint8_t sel,CDC_Xfer_TypeDef *cdc_Data);
...
{	
  ...
  if((pphost->device_prop.Itf_Desc[3].bInterfaceClass  == VENDOR_SPECIFIC)&&
    (pphost->device_prop.Itf_Desc[3].bInterfaceSubClass  == VENDOR_SPECIFIC) &&
      (pphost->device_prop.Itf_Desc[3].bInterfaceProtocol == VENDOR_SPECIFIC))
  {
    /*Collect the notification endpoint address and length*/
    CDC_Machine.CDC_CommItf.ep_addr = pphost->device_prop.Ep_Desc[3][0].bEndpointAddress;
    CDC_Machine.CDC_CommItf.length  = pphost->device_prop.Ep_Desc[3][0].wMaxPacketSize;
    
    if(pphost->device_prop.Ep_Desc[3][0].bEndpointAddress & 0x80)
    {
      CDC_Machine.CDC_CommItf.notificationEp = (pphost->device_prop.Ep_Desc[3][0].bEndpointAddress);
    }
      ...
      (pphost->device_prop.Itf_Desc[3].bInterfaceProtocol == VENDOR_SPECIFIC))
  {
    ...
    CDC_Machine.CDC_DataItf.ep_addr = pphost->device_prop.Ep_Desc[3][1].bEndpointAddress;
    CDC_Machine.CDC_DataItf.length  = pphost->device_prop.Ep_Desc[3][1].wMaxPacketSize;
    
    if(pphost->device_prop.Ep_Desc[3][1].bEndpointAddress & 0x80)
    {      
      CDC_Machine.CDC_DataItf.cdcInEp = (pphost->device_prop.Ep_Desc[3][1].bEndpointAddress);
    }
    else
    {
      CDC_Machine.CDC_DataItf.cdcOutEp = (pphost->device_prop.Ep_Desc[3][1].bEndpointAddress);
    }
    
    if(pphost->device_prop.Ep_Desc[3][2].bEndpointAddress & 0x80)
    {
      CDC_Machine.CDC_DataItf.cdcInEp = (pphost->device_prop.Ep_Desc[3][2].bEndpointAddress);
    }
    else
    {
      CDC_Machine.CDC_DataItf.cdcOutEp = (pphost->device_prop.Ep_Desc[3][2].bEndpointAddress);
    }
  ...
  /* GNSS Interface */
    if((pphost->device_prop.Itf_Desc[1].bInterfaceClass  == VENDOR_SPECIFIC)&&
      (pphost->device_prop.Itf_Desc[1].bInterfaceSubClass  == VENDOR_SPECIFIC) &&
        (pphost->device_prop.Itf_Desc[1].bInterfaceProtocol == VENDOR_SPECIFIC))
    {
      /*Collect the class specific endpoint address and length*/
      CDC_Machine.CDC_GNSSItf.ep_addr = pphost->device_prop.Ep_Desc[1][0].bEndpointAddress;
      CDC_Machine.CDC_GNSSItf.length  = pphost->device_prop.Ep_Desc[1][0].wMaxPacketSize;

      if(pphost->device_prop.Ep_Desc[1][0].bEndpointAddress & 0x80)
      {
        CDC_Machine.CDC_GNSSItf.cdcInEp = (pphost->device_prop.Ep_Desc[1][0].bEndpointAddress);
      }
      else
      {
        CDC_Machine.CDC_GNSSItf.cdcOutEp = (pphost->device_prop.Ep_Desc[1][0].bEndpointAddress);
      }

      if(pphost->device_prop.Ep_Desc[1][1].bEndpointAddress & 0x80)
      {
        CDC_Machine.CDC_GNSSItf.cdcInEp = (pphost->device_prop.Ep_Desc[1][1].bEndpointAddress);
      }
      else
      {
        CDC_Machine.CDC_GNSSItf.cdcOutEp = (pphost->device_prop.Ep_Desc[1][1].bEndpointAddress);
      }

      /*Allocate the length for host channel number out*/
      CDC_Machine.CDC_GNSSItf.hc_num_out = USBH_Alloc_Channel(pdev,
                                                              CDC_Machine.CDC_GNSSItf.cdcOutEp);
      /*Allocate the length for host channel number in*/
      CDC_Machine.CDC_GNSSItf.hc_num_in = USBH_Alloc_Channel(pdev,
                                                             CDC_Machine.CDC_GNSSItf.cdcInEp);

      /* Open channel for OUT endpoint */
      USBH_Open_Channel  (pdev,
                          CDC_Machine.CDC_GNSSItf.hc_num_out,
                          pphost->device_prop.address,
                          pphost->device_prop.speed,
                          EP_TYPE_BULK,
                          CDC_Machine.CDC_GNSSItf.length);
      /* Open channel for IN endpoint */
      USBH_Open_Channel  (pdev,
                          CDC_Machine.CDC_GNSSItf.hc_num_in,
                          pphost->device_prop.address,
                          pphost->device_prop.speed,
                          EP_TYPE_BULK,
                          CDC_Machine.CDC_GNSSItf.length);
                          ...
                          void *phost)
{
  ...
  if ( CDC_Machine.CDC_GNSSItf.hc_num_out)
    {
      USB_OTG_HC_Halt(pdev, CDC_Machine.CDC_GNSSItf.hc_num_out);
      USBH_Free_Channel  (pdev,CDC_Machine.CDC_GNSSItf.hc_num_out);
      CDC_Machine.CDC_GNSSItf.hc_num_out = 0;     /* Reset the Channel as Free */
    }

    if ( CDC_Machine.CDC_GNSSItf.hc_num_in)
    {
      USB_OTG_HC_Halt(pdev, CDC_Machine.CDC_GNSSItf.hc_num_in);
      USBH_Free_Channel  (pdev,CDC_Machine.CDC_GNSSItf.hc_num_in);
      CDC_Machine.CDC_GNSSItf.hc_num_in = 0;     /* Reset the Channel as Free */
    }
...
{   
  ...
  case CDC_SET_CONTROL_LINE_STATE_REQUEST:
    ...
    if( ClassReqStatus == USBH_OK )
    {
      ...
      CDC_GnssRxParam.CDCState = CDC_IDLE;
...
static void CDC_ProcessReception(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
{
  ...
  if(RX_Enabled == 1)
  {
    ...
    URB_STATE URB_StatusGNSSRx =   HCD_GetURB_State(pdev , CDC_Machine.CDC_GNSSItf.hc_num_in);
    ...
    switch(CDC_GnssRxParam.CDCState)
        {
        ...
        case CDC_IDLE:
          ...
          if(CDC_GnssRxParam.DataLength < (CDC_GnssRxParam.BufferLen - CDC_Machine.CDC_GNSSItf.length))
                {
                  /*Receive the data */
                  USBH_BulkReceiveData(pdev,
                		  	  	  	   CDC_GnssRxParam.pFillBuff,
                                       CDC_Machine.CDC_GNSSItf.length,
                                       CDC_Machine.CDC_GNSSItf.hc_num_in);

                  /*change the cdc state to USBH_CDC_GET_DATA*/
                  CDC_GnssRxParam.CDCState = CDC_GET_DATA;
                }
          break;
        ...
        case CDC_GET_DATA:
          ...
          /*Check the last state of the device is URB_DONE */
                if(URB_StatusGNSSRx == URB_DONE)
                {
                  /* Move the pointer as well as datalength */
                	CDC_GnssRxParam.DataLength += pdev->host.hc[CDC_Machine.CDC_GNSSItf.hc_num_in].xfer_count ;
                	CDC_GnssRxParam.pFillBuff += pdev->host.hc[CDC_Machine.CDC_GNSSItf.hc_num_in].xfer_count ;


                  /* Process the recived data */
                  CDC_ReceiveData(1,&CDC_GnssRxParam);
...
static void CDC_InitTxRxParam(void)
{
  ...
  /*Initialize the Receive buffer and its parameter*/
  CDC_GnssRxParam.CDCState = CDC_IDLE;
  CDC_GnssRxParam.DataLength = 0;
  CDC_GnssRxParam.pFillBuff = GnssRxBuf;
  CDC_GnssRxParam.pEmptyBuff = GnssRxBuf;
  CDC_GnssRxParam.BufferLen = sizeof(GnssRxBuf);
}
...
static void CDC_ReceiveData(uint8_t sel,CDC_Xfer_TypeDef *cdc_Data)
{
  ...
  if(cdc_Data->pEmptyBuff < cdc_Data->pFillBuff)
  {
    ...
    *ptr = 0x00;
    ...
    if(sel==1)
    UserCb.GnssReceive(cdc_Data->pEmptyBuff);
